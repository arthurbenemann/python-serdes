...
def mlse(signal, channel_response, traceback_length):
    ...
    # each row is a path ?
    path_mem = np.zeros((K, traceback_length), dtype=bool)
    ...
    for sample in signal:
        ...
        decisions = even_metrics < odd_metrics

        # Select
        state_metrics = np.where(
            decisions, even_metrics, odd_metrics)  # select lower metric

        # Update path memmory
        path_mem = np.roll(path_mem, 1)
        path_mem[:, 0] = decisions

        # Find most likely path
        likely_state = np.argmin(state_metrics)

        for decisions in path_mem:
            traceback_decision = int(decisions[likely_state])
        ...
        # print(path_mem, lowblue(state_metrics),   lowblue(branch_metrics), bit)
    return detections
